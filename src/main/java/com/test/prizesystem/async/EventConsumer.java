    ...
    public void processEvent(UserDrawEvent event) {
        ...
        if (event.getPrizeId() != null) {
            try {
                ...
                // 保存到用户中奖记录树
                String userTreeKey = TreeNames.USER_PRIZE_RECORDS.getTreeName() + ":" + event.getUserId();
                treeStorage.save(userTreeKey, record.getId(), record);
                
                // 保存到活动中奖记录树
                String activityTreeKey = TreeNames.ACTIVITY_PRIZE_RECORDS.getTreeName() + ":" + event.getActivityId();
                treeStorage.save(activityTreeKey, record.getId(), record);
